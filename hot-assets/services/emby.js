// Launch Tube: Emby External Player Integration
...
(function() {
    ...
    if (window.__LAUNCHTUBE_EMBY_LOADED__) return;
    ...
    async function detectEmbyVersion() {
        try {
            ...
            console.log('Launch Tube: Could not detect Emby version:', e);
        }
        return null;
    }
    ...
    async function tryLoadVersionedScript(version) {
        ...
        try {
            ...
            if (resp.ok && resp.status !== 204) {
                ...
                if (script && script.includes('__LAUNCHTUBE_EMBY_LOADED__')) {
                    console.log('Launch Tube: Loading versioned script for Emby', version);
                    window.__LAUNCHTUBE_EMBY_LOADED__ = true;
    ...
    async function bootstrap() {
        const version = await detectEmbyVersion();
        console.log('Launch Tube: Detected Emby version:', version);
    ...
    // Base implementation
    function initEmby() {
        console.log('Launch Tube: Emby script loaded');
    ...
    // Get Emby auth info from ApiClient
    function getServerInfo() {
        const serverUrl = window.location.origin;
        const userId = window.ApiClient?.getCurrentUserId?.() || '';
        const token = window.ApiClient?.accessToken?.() || '';
        serverLog(`getServerInfo: serverUrl=${serverUrl}, userId=${userId}, hasToken=${!!token}`);
        return { serverUrl, userId, token };
    }

    // Get item details from Emby API
    ...
    async function playExternal(itemId, startPositionTicks = 0) {
        serverLog(`playExternal called: itemId=${itemId}, startPositionTicks=${startPositionTicks}`);
        showModal('Loading...');

        try {
            const item = await getItemDetails(itemId);
            serverLog(`Item details: type=${item.Type}, name=${item.Name}, mediaSourceId=${item.MediaSources?.[0]?.Id}`);
            ...
            serverLog(`playExternal error: ${e.message}`, 'error');
            updateModalStatus('Failed to start player: ' + e.message);
            setTimeout(() => hideModal(false), 3000);
            return false;
        }
    ...
    function attachPlayListeners() {
        ...
        document.addEventListener('click', async function(event) {
            ...
            if (target) {
                ...
                if (itemId) {
                    serverLog(`Play button clicked, itemId: ${itemId}`);
                    // Prevent Emby from starting its player
                    event.preventDefault();
                    event.stopPropagation();
                    event.stopImmediatePropagation();
    ...
    // Hide menu items and video player UI
    function hidePlayMenuItems() {
        ...
        style.textContent = `
            ...
            .actionSheetMenuItem[data-id="resume"],
            .actionSheetMenuItem[data-id="playtrailers"],
            .listItem[data-action="play"],
            .listItem[data-action="playallfromhere"],
            .listItem[data-action="resume"],
            /* Hide menu items with play icon */
            .actionSheetMenuItem:has(.play_arrow),
            .actionSheetMenuItem:has(.md-icon):has([class*="play"]),
            .listItem:has(.play_arrow),
            /* Emby context menu buttons */
            button[data-id="play"],
            button[data-id="resume"],
            button[data-id="playallfromhere"],
            .btnPlay,
            .menuItem[data-command="play"] {
                display: none !important;
            }
            /* Always hide the web video player */
    ...
    // Hook PlaybackManager.play to intercept before Emby starts loading
    ...
    // Initialize
    function init() {
        serverLog('Initializing Emby integration');
        hidePlayMenuItems();
        hookPlaybackManager();
        attachPlayListeners();
